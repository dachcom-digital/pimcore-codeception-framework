...
touch $TEST_BUNDLE_TEST_DIR/_data/config/config.yaml

## move TestKernel
cp $PIMCORE_CODECEPTION_FRAMEWORK/src/Support/App/TestKernel.php $TEST_PROJECT_ROOT_DIR/src/TestKernel.php
...
for CURRENT_CONFIG_NODE in ${__}; do
  ...
  for FILE in ${!SECTIONS}; do
    ...
    rsync -aqz "$TEST_BUNDLE_TEST_DIR/_etc/config/$realK" "$TEST_PROJECT_ROOT_DIR/$realV"
